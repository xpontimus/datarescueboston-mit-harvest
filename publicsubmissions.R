...
for(i in 1) {
  ...
  if(url != "") {
    ...
    xls.output <- html_nodes(pg, "a") %>%
      html_attr("href") %>%
      grep("excel12book", ., value = TRUE)
    
    output <- c(pdf.output, doc.output, xls.output)
...
for(i in 1:length(pdflist)) {
  ...
  if(grepl("pdf", pdflist[i]) == TRUE) {
    ...
    print(outfile)
    if(length(pdflist[i]) > 0) {
      download.file(pdflist[i], destfile = paste("comments/", outfile, sep=""), method = "wininet", mode ="wb")
    }
  } else if (grepl("excel12book", pdflist[i]) == TRUE) {
    outfile <- strsplit(pdflist[i], "EPA")[[1]][2]
    outfile <- paste("EPA", strsplit(outfile, "&")[[1]][1], ".xlsx", sep="")
    print(outfile)
    if(length(pdflist[i]) > 0) {
      download.file(pdflist[i], destfile = paste("comments/", outfile, sep=""), method = "wininet", mode ="wb")
    } 
  } else if (grepl("docx", pdflist[i]) == TRUE) {
    outfile <- strsplit(pdflist[i], "EPA")[[1]][2]
    outfile <- paste("EPA", strsplit(outfile, "&")[[1]][1], ".docx", sep="")
    print(outfile)
    if(length(pdflist[i]) > 0) {
      download.file(pdflist[i], destfile = paste("comments/", outfile, sep=""), method = "wininet", mode ="wb")
    } 
